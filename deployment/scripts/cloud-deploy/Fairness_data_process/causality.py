    ...
    for name in experiment_names:
        ...
        for num in experiment_nums:
            ...
            for i in peer_dir:
                ...
                pattern_glo_prose=r'Sending Global-PREPREPARE.'
                # pattern_commit=r'Get logEntry.Sn from tn'
                pattern_commit=r'Handling Global-COMMIT'
                ...
                for line in lines:
                    ...
                    if find_ is not None:
                        # print(find_)
                        find_sn=re.search(r'gsn=\d+',line)
                        if find_sn is None:
                            continue
                        sn=int(line[find_sn.span()[0]+4:find_sn.span()[1]])
                        if sn not in sn_commit:
                            sn_commit[sn]=[cast_time(line)]
                        else:
                            sn_commit[sn].append(cast_time(line))   

                    find_=re.search(pattern_glo_prose,line)
                    if find_ is not None:
                        find_sn=re.search(r'gsn=\d+',line)
