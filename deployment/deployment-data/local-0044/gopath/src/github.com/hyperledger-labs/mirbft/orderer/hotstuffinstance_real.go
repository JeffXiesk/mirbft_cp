...
type hotStuffInstance_real struct {
...
func (hi *hotStuffInstance_real) init(seg manager.Segment, orderer *HotStuffOrderer) {
	...
	htnlog[hi.segment.SegID()] = 0
}

func (hi *hotStuffInstance_real) start() {
...
func (hi *hotStuffInstance_real) proposeSN(sn int32) {
	hi.hi_proxy.proposeSN(sn)
}
...
func (hi *hotStuffInstance_real) proposeSN_real(sn int32) {
...
func (hi *hotStuffInstance_real) handleProposal(proposal *pb.HotStuffProposal, msg *pb.ProtocolMessage) error {
	sn := msg.Sn
	...
	if  (senderID+msg.FakeView)%int32(len(hi.segment.Followers())) != proposal.Leader {
		hi.sendNewView()
		return fmt.Errorf("malformed message: sender %d and fake_view %d does not match leader %d", senderID, msg.FakeView, proposal.Leader)
	}
	...
	// Check we have already voted for the certificate height to use Fcertificate as a parent.
	...
	if !decided.announced {
		hi.announce(decided, decided.Tn, decided.batch.Message(), decided.digest, decided.node.Aborted)
	}
...
func (hi *hotStuffInstance_real) sendVote(node *hotStuffNode, dst int32) {
	...
	if node.Tn > htnlog[hi.segment.SegID()] {
		logger.Debug().Int32("Origin hightimestamp", htnlog[hi.segment.SegID()]).
			Int32("New hightimestamp", node.Tn).
			Int("segment",hi.segment.SegID()).
			Msg("Update new high time stamp.")
		htnlog[hi.segment.SegID()] = node.Tn
		// hi.TsQc = signed.TsQc
	} else {
		logger.Debug().Int32("Origin hightimestamp", htnlog[hi.segment.SegID()]).
			Int32("New hightimestamp", node.Tn).
	...
	for j:=0;j<(len(hi.segment.Followers()));j++ {
...
func (hi *hotStuffInstance_real) handleVote(signed *pb.HotstuffSignedMsg, sn, senderID, hightimestamp int32) error {
	vote := &pb.HotStuffVote{}
	...
	prehtn := htnlog[hi.segment.SegID()]
	logger.Info().Int32("sn", sn).
		Int("segment", hi.segment.SegID()).
		Int32("height", vote.Height).
		Int32("Origin hightimestamp", prehtn).
	...
	// } else {
	// 	if hightimestamp > htnlog[hi.segment.SegID()] {
	// 		logger.Debug().Int32("Origin hightimestamp", htnlog[hi.segment.SegID()]).
	// 			Int32("New hightimestamp", hightimestamp).
	// 			Msg("Update new high time stamp.")
	// 		htnlog[hi.segment.SegID()] = hightimestamp
	// 		hi.TsQc = signed.TsQc
	// 	} else {
	// 		logger.Debug().Int32("Origin hightimestamp", htnlog[hi.segment.SegID()]).
	// 			Int32("New hightimestamp", hightimestamp).
	...
	if hightimestamp > prehtn {
		logger.Debug().Int32("Origin hightimestamp", prehtn).
			Int32("New hightimestamp", hightimestamp).
			Msg("Update new high time stamp.")
		htnlog[hi.segment.SegID()] = hightimestamp
		// hi.TsQc = signed.TsQc
	...
	if hi.leaf.height < vote.Height {
		...
		logger.Debug().
			Msgf("vote from normal height %d have been received, we are in height %d", vote.Height, hi.leaf.height)
	}
...
func (hi *hotStuffInstance_real) sendNewView() {
	// Advance view
	hi.view += 1
...
func (hi *hotStuffInstance_real) handleNewView(newview *pb.HotStuffNewView, sn, senderID int32) error {
	logger.Info().Int32("sn", sn).
		Int("segment", hi.segment.SegID()).
		Int32("height", newview.Certificate.Height).
		Int32("senderID", senderID).
...
func (hi *hotStuffInstance_real) handleMissingEntry(msg *pb.MissingEntry) {
	logger.Info().
		Int("segment", hi.segment.SegID()).
		Int32("view", hi.view).
		Int32("sn", msg.Sn).
...
func (hi *hotStuffInstance_real) announce(node *hotStuffNode, tn int32, reqBatch *pb.Batch, digest []byte, aborted bool) {
	if node != nil {
		// Cancel timer
	...
	logger.Info().
		...
		Int32("newSn",int32(len(hi.segment.Followers()))*(tn-1)+int32(hi.segment.SegID())).
		Int32("height", node.height).
		Msg("Announcement.")

	logEntry := &log.Entry{
		Sn:      int32(len(hi.segment.Followers()))*(tn-1)+int32(hi.segment.SegID()),
		Batch:   reqBatch,
		Aborted: aborted,
		Digest:  digest,
	}
	...
	logger.Debug().
		Int32("tn",htnlog[hi.segment.SegID()]).
		Int32("start_tn",hi.height2tn[node.height]).
		Int32("end_tn",hi.height2tn[node.height-1]).
		Int32("sn",int32(len(hi.segment.Followers()))*(tn-1)+int32(hi.segment.SegID())).
		Msg("Check Announce.")
...
// Creates a new node
func (hi *hotStuffInstance_real) newNode(parent *hotStuffNode, batch *request.Batch, qc *pb.HotStuffQC, sn int32, height int32, leader int32) *hotStuffNode {
	node := &pb.HotStuffNode{
		View:   hi.view,
	...
	new := &hotStuffNode{
		...
		dummy:  false,
		Tn:		htnlog[hi.segment.SegID()]+1,
	}
	hi.height2tn[new.height] = new.Tn
	
	logger.Debug().Int("segment",hi.segment.SegID()).Msgf("Height2tn content: %v",hi.height2tn)
	logger.Debug().Int("segmentID",hi.segment.SegID()).Int32("height",new.height).Int32("Tn",new.Tn).Msg("height2tn new content.")
	if _, ok := hi.sn2height[sn]; ok {
		new.dummy = true
	}
	logger.Debug().
...
func (hi *hotStuffInstance_real) addNode(node *hotStuffNode) {
	logger.Debug().
		Int("segment", hi.segment.SegID()).
		Int32("sn", node.sn).
		Int32("height", node.height).
...
func (hi *hotStuffInstance_real) updateHighQC(qc *pb.HotStuffQC) {
	if qc.Height > hi.highQC.Height {
		...
		logger.Debug().Int32("qc.Height",qc.Height).Int32("hi.highQC.Height",hi.highQC.Height).Msg("Not enter the updateHighQC block")
	}
}
...
func (hi *hotStuffInstance_real) setViewChangeTimer(sn int32, node *hotStuffNode) {
	if node.viewChangeTimer != nil {
		logger.Debug().
			Int("segment", hi.segment.SegID()).
			Int32("sn", sn).
...
func (hi *hotStuffInstance_real) processSerializedMessages() {
	logger.Debug().
		Int("segment", hi.segment.SegID()).
...
func (hi *hotStuffInstance_real) subscribeToBacklog() {
...
func (hi *hotStuffInstance_real) stopProposing() {
	hi.stopProp.Do(func() {
		close(hi.newBatch)
	})
}

func (hi *hotStuffInstance_real) startView(view int32) {
	if hi.view > view {
