...
func (pi *pbftInstance) init(seg manager.Segment, orderer *PbftOrderer) {
	...
	htnssn[int32(pi.segment.SegID())] = append(htnssn[int32(pi.segment.SegID())],0)///1201
		...
		msg := &pb.ProtocolMessage{
			...
			Msg: &pb.ProtocolMessage_Newseqno{
				Newseqno: &pb.PbftPreprepare{
					...
					Tn: htnlog[pi.segment.SegID()]+1,//10240922
				},
			},
		}
...
///1201
func (pi *pbftInstance) sendproposal(sn int32, batchSize int) {
		// Wait for a batch to be ready.
		// We must not cut the batch now, as, in case of a view change,
		// it might get stuck in the instance serializer buffer without being processed.
		// The actual batch cutting happens when handling this event (and thus on the critical path of the instance).
...
func (pi *pbftInstance) handlePreprepare(preprepare *pb.PbftPreprepare, msg *pb.ProtocolMessage) error {
	...
	///1201
	for _,i := range htnssn[sn] {
		logger.Info().
		Int32("htnssn", i).
		Msg("htnssn.")
	}

	if tn < GetMaxHtn(htnssn[sn]) {
...
///1201
func GetMaxHtn(ary []int32) int32 {
	if len(ary) == 0 {
		return 0
	}
	...
	maxVal := ary[0]
	for i := 1; i < len(ary); i++ {
		logger.Info().Int32("htnssn", ary[i]).
		Msg("htnssn sets.")
		if maxVal < ary[i] {
			maxVal = ary[i]
		}
...
func (pi *pbftInstance) handleHtnmsg(htnmsg *pb.HtnMessage, msg *pb.ProtocolMessage) error {
	...
	htnssn[htnmsg.Sn+int32(membership.NumNodes())] = append(htnssn[htnmsg.Sn+int32(membership.NumNodes())], htnmsg.Htn)
