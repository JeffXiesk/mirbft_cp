...
with open('scripts/cloud-deploy/cloud-instance-info','w') as f:
    f.write('master '+sys.argv[3]+' '+sys.argv[num+3]+' master us-east-1a\n')
    client_str='1client'
    if client_num==16:
    ...
    for i in range(client_num):
        f.write('client '+sys.argv[i+4]+' '+sys.argv[num+4+i]+' '+client_str+' us-east-1a\n')
    for i in range(peer_num):
        f.write('p'+str(i+1)+' '+sys.argv[i+client_num+4]+' '+sys.argv[client_num+num+4+i]+' peers us-east-1a\n')
