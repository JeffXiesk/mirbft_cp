...
var(
	htnlog map[int]int32
	height2tn [4]map[int32]int32 // Height to timestamp number
)
...
func (hi *hotStuffInstance) init(seg manager.Segment, orderer *HotStuffOrderer) {
	...
	for i:=0; i<4; i++ {
		height2tn[i] = make(map[int32]int32)
	}
...
func (hi *hotStuffInstance) proposeSN(sn int32) {
	...
	height2tn[hi.segment.SegID()][hi.leaf.height+1] = htnlog[hi.segment.SegID()]+1
	// htnlog[hi.segment.SegID()] = htnlog[hi.segment.SegID()] + 1

	for j:=0; j<4;j++{
		logger.Debug().
			Int("j",j).
			Int32("now time",htnlog[j]).
			Msg("Show htnlog content.")
	}
	...
	msg := &pb.ProtocolMessage{
		...
		Hightimestamp: height2tn[hi.segment.SegID()][new.height],
	}
	...
	hi.handleProposal(msg.GetProposal(), msg, height2tn[hi.segment.SegID()][new.height])
...
func (hi *hotStuffInstance) handleProposal(proposal *pb.HotStuffProposal, msg *pb.ProtocolMessage, hightimestamp int32) error {
	sn := msg.Sn
	...
	logger.Debug().
		Int32("proposal.Node.Certificate.Height",proposal.Node.Certificate.Height).
		Int32("proposal.node.height",proposal.Node.Height).
		Msg("newnode in handle proposal")
	...
	// TODO: Fix
	height2tn[senderID][proposal.Node.Height] = tn
	...
	if !decided.announced {
		hi.announce(decided, tn, decided.batch.Message(), decided.digest, decided.node.Aborted)
	}
...
func (hi *hotStuffInstance) sendVote(node *hotStuffNode) {
	...
	msg := &pb.ProtocolMessage{
		...
		Hightimestamp: height2tn[hi.segment.SegID()][node.height],
		// Hightimestamp: decided.height,
	}
	
	// if msg.Hightimestamp > htnlog[hi.segment.SegID()] {
	// 	logger.Debug().Int32("Origin hightimestamp", htnlog[int(membership.OwnID)]).
	// 		Int32("New hightimestamp", hightimestamp).
	// 		Msg("Update new high time stamp in send vote.")
	// 	htnlog[hi.segment.SegID()] = msg.Hightimestamp
	...
	logger.Info().Int32("sn", hi.height2sn[node.height]).
		Int("segment", hi.segment.SegID()).
		Int32("height", node.height).
		Int32("SendID",membership.OwnID).
		Int32("leader", hi.leader).
		Int32("Hightimestamp",htnlog[int(membership.OwnID)]).
		Msg("Sending VOTE.")
...
func (hi *hotStuffInstance) handleVote(signed *pb.HotstuffSignedMsg, sn, senderID, hightimestamp int32) error {
	...
	logger.Info().Int32("sn", sn).
		...
		Int32("Origin hightimestamp", htnlog[int(membership.OwnID)]).
...
func (hi *hotStuffInstance) handleMissingEntry(msg *pb.MissingEntry) {
	...
	if !ok {
		hi.announce(nil, height2tn[hi.segment.SegID()][height], msg.Batch, msg.Digest, msg.Aborted)
	} else {
		...
		if !node.announced {
			...
			hi.announce(node, height2tn[hi.segment.SegID()][height], msg.Batch, msg.Digest, msg.Aborted)
		}
	}
}
...
func (hi *hotStuffInstance) announce(node *hotStuffNode, tn int32, reqBatch *pb.Batch, digest []byte, aborted bool) {
	...
	for j:=int32(0); j<=node.height; j++ {
		logger.Debug().
			Int32("height", j).
			Int32("tn",height2tn[hi.segment.SegID()][int32(j)]).
			Msg("height2tn content.")
	}

	start_tn := height2tn[hi.segment.SegID()][node.height]
	end_tn := height2tn[hi.segment.SegID()][node.height-1]
	logger.Debug().
		Int32("tn",htnlog[int(membership.OwnID)]).
		Int32("start_tn",height2tn[hi.segment.SegID()][node.height]).
		Int32("end_tn",height2tn[hi.segment.SegID()][node.height-1]).
		Int32("sn",4*(tn-1)+int32(hi.segment.SegID())).
		Msg("Check Announce.")
	for tt := start_tn-1; tt>end_tn&&end_tn>0; tt-- {
		logger.Debug().
			Int32("start_tn",height2tn[hi.segment.SegID()][node.height]).
			Int32("end_tn",height2tn[hi.segment.SegID()][node.height-1]).
			Int32("sn",4*(tt-1)+int32(hi.segment.SegID())).
			Msg("Announce Nil!")
		logEntry1 := &log.Entry{
			Sn:        4*(tt-1)+int32(hi.segment.SegID()),
			Batch:     reqBatch,
			Aborted:   aborted,
			Digest:    digest,
		}
...
func (hi *hotStuffInstance) processSerializedMessages() {
	...
	for msg := range hi.serializer.channel {
		...
		case *pb.ProtocolMessage_Proposal:
			...
			proposal := m.Proposal
			err := hi.handleProposal(proposal, msg, msg.Hightimestamp)
			if err != nil {
				logger.Error().
					Err(err).
					Int("segment", hi.segment.SegID()).
