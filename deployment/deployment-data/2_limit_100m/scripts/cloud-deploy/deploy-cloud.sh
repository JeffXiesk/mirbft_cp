...
do
    ssh $ssh_options_cloud root@${public_ip_arr[c]} 'tc qdisc del dev eth0 root;tc qdisc add dev eth0 root tbf rate 100mbit burst 320kbit latency 100ms'
    echo ${public_ip_arr[c]} '100mbit'
    # Limiting the Egress Traffic
done
for ((c=bandwidth_cnt;c<$peer_num;c++))
do
    ssh $ssh_options_cloud root@${public_ip_arr[c+bandwidth_cnt]} 'tc qdisc del dev eth0 root;tc qdisc add dev eth0 root tbf rate 1000mbit burst 320kbit latency 100ms'
    echo ${public_ip_arr[c+bandwidth_cnt]} '1000mbit'
done
