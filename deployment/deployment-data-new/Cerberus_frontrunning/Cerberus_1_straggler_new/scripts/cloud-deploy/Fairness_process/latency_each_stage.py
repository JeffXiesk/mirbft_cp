    ...
    for num in experiment_num:
        ...
        for i in client_dir:
            ...
            for line in lines:
                ...
                find_2=re.search(pattern_finish,line)
                if find_2 is not None:
                    # print(line)
                    req_num=int(line[find_2.span()[1]:].strip('\n'))
        ...
        for i in req_info.keys():
            sn=req_info[i]['sn']
            # print(sn)
            # 当实验没跑完时，有的request是还没有finish的
            if (i in req_submit.keys() and i in req_finish.keys()):
                base = req_submit[i]
            else:
                continue
